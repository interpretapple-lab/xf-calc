...
class TrapecioJiMa(FuzzyNumber):
    ...
    def multiplicacion(self, trap):
        a = ((3/2)*(self.b-self.a)*(trap.b-trap.a)) + (2*(trap.a*(self.b - self.a) + self.a*(trap.b - trap.a))) + (3*trap.a*self.a) - (2*self.b*trap.b)
        b = self.b * trap.b
        c = self.c * trap.c
        d = ((3/2)*(self.d-self.c)*(trap.d-trap.c)) - (2*((self.d-self.c)*trap.c+(trap.d-trap.c)*self.d)) + (3*self.d*trap.d) - (2*self.c*trap.c)
...
class TrapecioTaRe(FuzzyNumber):
    ...
    def multiplicacion(self, trap):
        ...
        C2 = Symbol('C2')
        D2 = Symbol('D2')
        a = (A1*A2).subs({A1:self.a, A2:trap.a})
        b = (B1*B2).subs({B1:self.b, B2:trap.b})
        c = (C1*C2).subs({C1:self.c, C2:trap.c})
        d = (D1*D2).subs({D1:self.d, D2:trap.d})
